...
if [[ ! -f "$CSV" && "$DRY_RUN" -eq 0 ]]; then
  echo "file,impl,tau,threads,maxbuf,memlimit_mb,vars,edges,total_sec,parse_sec,vig_build_sec,agg_memory" > "$CSV"
fi
...
if [[ -n "$REUSE_CSV" ]]; then
  ...
  # Iterate safely even if basenames is unset under set -u
...
if [[ "$SKIP_EXISTING" -eq 1 ]]; then
  if [[ -f "$CSV" ]]; then
    ...
    # header: file,impl,tau,threads,maxbuf,...
    tail -n +2 "$CSV" | awk -F, '{print $1","$2","$3","$4","$5}' | sort -u > "$KEYS_FILE"
...
run_case() {
  local filepath="$1" display_base="$2" impl="$3" tau="$4" threads="$5" maxbuf="$6" memlimit_mb="$7"
  ...
  vprint "PLAN file=$display_base impl=${impl#--} tau=$tau threads=$threads maxbuf=${maxbuf:-} memlimit_mb=${memlimit_mb:-}"

  if [[ "$DRY_RUN" -eq 1 ]]; then
    # Print a representative command (not exact quoting) without executing.
    if [[ "$filepath" == *.xz ]]; then
  echo "DRY RUN: xz -dc -- '$display_base' | $BIN -i - --tau $tau ${impl} ${threads:+-t $threads} ${maxbuf:+--maxbuf $maxbuf}" >&2
    else
  echo "DRY RUN: $BIN -i '$display_base' --tau $tau ${impl} ${threads:+-t $threads} ${maxbuf:+--maxbuf $maxbuf}" >&2
    fi
    return 0
  fi
  ...
  # We require vars, edges, total_sec, impl, tau, threads, agg_memory.
  local summary
  summary=$(grep -E "vars=[0-9]+ .*edges=[0-9]+ .*total_sec=[0-9.]+ .*impl=(naive|opt) .*tau=(-1|[0-9]+) .*threads=(-1|[0-9]+) .*agg_memory=[0-9]+" "$log" | tail -n 1 || true)
  if [[ -n "$summary" ]]; then
    # Extract fields
    local vars edges total_sec parse_sec vig_build_sec impl_out tau_out threads_out agg
    ...
    echo "$base,$impl_out,$tau_out,$threads_out,$maxbuf,${memlimit_mb:-},$vars,$edges,$total_sec,$parse_sec,$vig_build_sec,$agg" >> "$CSV"
    ok=1
  else
